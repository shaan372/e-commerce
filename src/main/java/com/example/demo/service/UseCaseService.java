...
public class UseCaseService {
    ...
    public List<UseCase> findAllUseCasesByIds(List<Integer> ids){
        List<UseCase> useCases = StreamSupport
                .stream(useCaseRepository.findAllById(ids).spliterator(), false)
                .toList();
        return useCases;
    }

    public List<UseCase> saveAllUseCases(List<UseCaseRequest> useCaseRequests){
        List<UseCase> useCases = useCaseRequests.stream()
                .map((useCaseRequest -> {
                    UseCase useCase = new UseCase();
                    useCase.setDescription(useCaseRequest.getDescription());
                    return useCase;
                })).toList();
        List<UseCase> useCaseList = StreamSupport
                .stream(useCaseRepository.saveAll(useCases).spliterator(), false)
                .toList();
        return useCaseList;
    }
}
